...
DATA_FILE = '2025_06.csv'  # Your CSV file path
MAX_GAP_MINUTES = 280
...
def load_and_prepare_data(filepath: str) -> pd.DataFrame:
    ...
    # Read the CSV file with the correct delimiter (comma-separated)
    df = pd.read_csv(filepath, sep=',', engine='python')
    
    # Print columns to verify if the data is correctly loaded
    print(f"Spaltennamen: {df.columns}")  # Überprüft die Spaltennamen
    
    # Ensure that the 'date' column exists and is correctly converted to datetime
    ...
    # Drop rows where the 'timestamp' column has invalid values (NaT)
    df.dropna(subset=['timestamp'], inplace=True)

    # Process each day's data to handle the 21:30 cutoff
    for date, group in df.groupby('date'):
        # Filter rows for the day before 21:30
        valid_before_2130 = group[group['timestamp'].dt.time <= datetime.time(21, 30)]
        
        # Get the last valid price before 21:30
        if not valid_before_2130.empty:
            last_valid_price = valid_before_2130['diesel'].iloc[-1]
            
            # Find any record at 22:00 and replace the price with the last valid price before 21:30
            if any(group['timestamp'].dt.time == datetime.time(22, 0)):
                df.loc[group['timestamp'].dt.time == datetime.time(22, 0), 'diesel'] = last_valid_price
...
def plot_daily_lows(df: pd.DataFrame, df_min: pd.DataFrame, interval_dict: dict):
    ...
    for bar, price, date, label_str in zip(bars, df_min['diesel'], df_min['date'], df_min['date_str']):
        ...
        # Displaying the label in the middle of the bar
        plt.text(
            bar.get_x() + bar.get_width() / 2,
            price - 0.025,  # slightly below top of bar
            label,
            ha='center',
            va='center',
            fontsize=14,
        ...
        if not day_data.empty:
            ...
            for i in range(0, len(times), len(times)//3):  # To avoid overlap, show fewer labels
    ...
    # Final adjustments
    plt.ylim(1.3, df_min['diesel'].max() + 0.02)
