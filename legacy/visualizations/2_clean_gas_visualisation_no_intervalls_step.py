...
DATA_FILE = '2025_06_27_ludgeri.csv'  # Your CSV file path
MAX_GAP_MINUTES = 800
...
def load_and_prepare_data(filepath: str) -> pd.DataFrame:
    ...
    df['date'] = df['timestamp'].dt.date  # Convert timestamp to date only
...
def plot_daily_lows(df: pd.DataFrame, df_min: pd.DataFrame):
    ...
    for bar, price, date, label_str in zip(bars, df_min['diesel'], df_min['date'], df_min['date_str']):
        ...
        # Price development line per day (raw data as stepped)
        ...
        if not day_data.empty:
            ...
            # Draw as step plot instead of linear line
