...
DATA_FILE = r'C:\Users\miche\OneDrive\Dokumente\3. Privat\VSC_Projects\tankerkoenig-data\prices\2025\06\2025-06-20-prices.csv'  # Adjust the path to the new file
MAX_GAP_MINUTES = 280

def load_and_prepare_data(filepath: str, station_uuid: str) -> pd.DataFrame:
    ...
    # Filter by station_uuid
    df = df[df['station_uuid'] == station_uuid]

    # Add a date column
    df['date'] = df['timestamp'].dt.date

    # Only keep data for the last 14 days
...
def plot_daily_lows(df: pd.DataFrame, df_min: pd.DataFrame, interval_dict: dict):
    ...
    for bar, price, date, label_str in zip(bars, df_min['diesel'], df_min['date'], df_min['date_str']):
       ...
       # plt.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.003,
        ...
        plt.text(
            ...
            va='center',
            fontsize=14,
            color='black'  # or 'black' if bars are light
)
    ...
    # Final adjustments
    plt.ylim(1.5, df_min['diesel'].max() + 0.12)
...
def show_daily_lows():
    station_uuid = '916d61b6-7279-4d63-a754-ae160f8cdee2'  # Station ID you're looking for
    df = load_and_prepare_data(DATA_FILE, station_uuid)
